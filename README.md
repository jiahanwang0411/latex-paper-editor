...
本项目采用**纯 HTML 单文件**设计（约 2100 行），天然适合与 **Codex、Claude Code、Kimi Work、QoderWork** 等 AI 编程工具协同工作：

- **零构建**：不需要 `npm install`、`webpack` 等任何构建步骤，AI 改完刷新浏览器即见效果
- **单文件即完整应用**：所有 CSS、JS、HTML 内联在一个文件中，AI 只需理解一个文件就能添加功能或修复问题
- **AI 初稿一键导入**：AI 生成的论文初稿（JSON 格式）可通过 Import 导入编辑器，自动按期刊格式渲染，用户直接在编辑器中精修——实现 "AI 写初稿 → 编辑器排版 → 人工润色" 的高效工作流
- **轻松扩展**：添加新期刊模板只需一条配置 + 几行 CSS，AI 工具可以秒级完成

> 详见下方 [与 AI 编程工具协同工作](#与-ai-编程工具协同工作) 章节
